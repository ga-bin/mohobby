      ...
      <v-card-actions>
        <v-col cols="10">
          <v-text-field class="m1-11" placeholder="댓글을 입력해주세요!" counter filled rounded solo dense hide-details
            v-model="inputCmt" name="content" @keydown.enter="enter()" />
        </v-col>
        <v-spacer></v-spacer>
        <!-- 댓글등록 버튼 -->
    ...
    <div>
      ...
      <div class="profile" v-for="cmt in comments" :key="cmt.commId">
        ...
        <div class="user text-overline">{{cmt.memberId}}
          ...
          <!-- 버튼: 저장, 답장, 수정, 삭제 -->
          <div class="btn">
            <v-btn x-small outlined color="success" class="mr-3" v-if="cmt.memberId == memberId"
              @click="editCmt(cmt.commId)">저장</v-btn>
            <v-btn x-small outlined color="dark-grey" class="mr-3" v-if="cmt.commId != editForm"
              @click="showRegReCmt(cmt.commId, cmt.memberId)">답장</v-btn>
            <v-btn x-small outlined color="success" class="mr-3" v-if="cmt.memberId == memberId"
              @click="showEditForm(cmt.commId)">수정</v-btn>
            <v-btn x-small outlined color="error" v-if="cmt.memberId == memberId"
              @click="deleteCmt(cmt.commId, cmt.targetId)">삭제</v-btn>
          </div>
          <!-- 대댓 유저소환 -->
          <v-card-actions>
            <div class="content">
              <div v-if='cmt.parentCommId != ""'>
                <span class="member_id"
                  @click="$router.push({ path: '/snsUserFeed?memId='+cmt.parentMemberId}).catch(()=>{$router.go(0)})"><strong>@{{cmt.parentMemberId}}</strong></span>{{cmt.content}}
              </div>
              ...
              <div v-if="cmt.commId == editForm && cmt.commId != reCmt && formValue">
                <v-textarea name="editContent" auto-grow outlined :value="originContent"
                  @keydown.enter="editEnter(cmt.commId)" v-model="editedContent"></v-textarea>
              </div>
            </div>
          </v-card-actions>
            ...
            <v-card-actions>
              <v-col cols="10">
                <v-text-field class="m1-11" filled rounded solo dense hide-details :prefix="cmtMemberId"
                  v-model="inputReCmt" name="content" @keydown.enter="reEnter(cmt.commId, cmt.memberId)" />
              </v-col>
              <v-spacer></v-spacer>
              <div style="margin-right: 80px">
...
export default {
  ...
  methods: {
    ...
    regCmt() {
      ...
      this.stompClient.send("/app/NoticeSns", JSON.stringify(noticeContent), res => {
        console.log(res)
      });
      }).catch(err => {
        console.log(err)
