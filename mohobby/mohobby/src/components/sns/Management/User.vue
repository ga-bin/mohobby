          ...
          <template>
            <v-list-item>
              <v-list-item-avatar>
                ...
                <v-img
                  v-if="followType === 1"
                  @click="goUserProfile(user.followingId)"
                  :src="require(`@/assets/image/user/${user.profileImg}`)"
                />

                <!-- 팔로잉 목록 클릭시 -->
                <v-img
                  v-else-if="followType === 2"
                  @click="goUserProfile(user.followerId)"
                  :src="require(`@/assets/image/user/${user.profileImg}`)"
                />

                <!-- 유저검색 -->
                <v-img
                  v-else-if="followType === 3"
                  @click="goUserProfile(user.memberId)"
                  :src="require(`@/assets/image/user/${user.profileImg}`)"
                />
              </v-list-item-avatar>
              <v-list-item-content>
              ...
              <div v-if="followType == 1">
                <v-btn
                  v-if="followerCheck[i] == 1"
                  @click="unfollow(user.followerId, user.followingId)"
                  rounded
                  color="#F36A3E"
                  class="white--text"
                  >언팔로우</v-btn
                ...
                <v-btn
                  v-else-if="followerCheck[i] == 0"
                  @click="followup(user.followingId, user.followerId)"
                  rounded
                  color="#2255b1"
                  class="white--text"
                  >팔로우</v-btn
              ...
              <!-- <div v-else>
                <v-btn @click="unfollow(user.followerId,user.followingId)" rounded color="#2ac187" class="white--text"> 언팔로우
                </v-btn>
              </div> -->
            </v-list-item>
          </template>
...
export default {
  ...
  methods: {
    ...
    followup(memberId, userId) {
      (this.follower = []), console.log("memberId : " + memberId);
      console.log("userId : " + userId);

      this.axios
        .post("/sns/follow", {
          followerId: userId, //로그인세션 아이디
          followingId: memberId, //피드주인 아이디
        })
        .then((res) => {
          // this.followStatus = 1;
