          ...
          <template v-slot:activator="{ on, attrs }" class="mb-16">
            ...
            <div>
              <v-btn
                ...
                color="#2255b1"
                class="white--text"
                >설정 <v-icon>mdi-cog-off</v-icon></v-btn
              >
              <v-btn v-else @click="manager()" color="#e9e9ec"
                >설정 <v-icon color="#2255b1" size="30">mdi-cog</v-icon></v-btn
              >
            </div>
          </template>
          <v-card class="mx-auto">
            <v-card-title>
              <span v-if="editMode == true" class="mx-auto text-h8"
                >컬렉션 수정</span
              >
            ...
            <v-card-actions v-if="editMode == true">
              ...
              <v-btn
                v-if="
                  editedCatgName.length >= 1 &&
                  editedCatgName.length <= 10 &&
                  catgNames.indexOf(editedCatgName) == -1 &&
                  checkSpace(editedCatgName) == false &&
                  checkSpecial(editedCatgName) == false
                "
                color="blue darken-1"
                text
                @click="editCollection(catgId, memberId)"
                >저장</v-btn
              >
            ...
            <v-card-actions v-else>
              ...
              <v-btn
                v-if="
                  newCatgName.length >= 1 &&
                  newCatgName.length <= 10 &&
                  catgNames.indexOf(newCatgName) == -1 &&
                  checkSpace(newCatgName) == false &&
                  checkSpecial(newCatgName) == false
                "
                color="blue darken-1"
                text
    ...
    >
      <div class="content">
        ...
        <v-img
          ...
          @click="goBookmark(collection.catgId, collection.catgName)"
          :src="
            require(`@/assets/image/sns/${collection.postId}/${collection.thumbnail}`)
          "
        />
        ...
        <div v-if="management == true" class="content-details fadeIn-bottom">
          <v-btn
            v-if="collection.catgName != 'default' && editMode == true"
            class="text--white"
            color="#2255b1"
            dark
            v-bind="attrs"
            v-on="on"
            @click="showEditForm(collection.catgId)"
            >수정</v-btn
          >
          ...
          <v-btn
            ...
            dark
            color="#F36A3E"
            @click="swal(collection.memberId, collection.catgId)"
            >삭제</v-btn
          >
          <h3 v-if="collection.catgName == 'default'" class="content-title">
            {{ collection.catgName }}
          </h3>
        </div>
  ...
  data() {
    return {
      ...
      rules: [
        ...
        (value) =>
          (value && value.length <= 10) || "10글자 이내로 부탁드립니다🙏", //10글자 이내이면
        (value) =>
          this.catgNames.indexOf(value) == -1 || "이미 존재하는 이름입니다🙏", //이미 존재하는 이름이면
        (value) =>
          this.checkSpace(value) == false || "공백은 자제 부탁드립니다🙏", //공백이 있으면
        (value) =>
          this.checkSpecial(value) == false ||
          "특수문자는 사용 자제 부탁드립니다🙏", //특수문자가 있으면
      ],
    };
  },
  setup() {},
  ...
  methods: {
      ...
      this.axios("/sns/collection/" + memberId, {
        ...
        .then((res) => {
          if (res.data.length == 0) {
            // 만들어놓은 컬렉션이 존재하지 않으면 디폴트 컬렉션 생성
            this.createDefaultCollection(memberId);
          } else {
            // 만든 컬렉션이 존재하면 이름만 뽑아서 바인딩
    ...
    goBookmark(catgId, catgName) {
      ...
      this.$router.push({
        ...
        query: { catgId: catgId, catgName: catgName },
      });
    },
  },
};
