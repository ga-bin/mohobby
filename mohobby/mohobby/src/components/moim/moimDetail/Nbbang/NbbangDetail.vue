    ...
    <v-card class="mx-auto" max-width="1000">
      ...
      <div>
        <div class="flex">
          ...
          <div class="user text-overline">{{writer}}<br>{{items.writeDate}}</div>
        </div>
      </div>
            ...
            <v-list two-line>
              <div class="">
                <template v-for="(item, index) in items.slice(0, 6)">
                  ...
                  <v-subheader v-if="item.title" :key="items.title" class="mr-5">
                    <v-icon class="mr-1 mt-1 ml-3" color="light-green">mdi-chart-pie</v-icon>
                    {{ item.title }}
                    {{item.totalPrice}}/{{item.people}}명
                  </v-subheader>
                  <v-divider v-else-if="item.divider" :key="index" :inset="item.inset"></v-divider>
                  <v-list-item v-else :key="items.memberId">
                    <v-list-item-avatar>
                      <img :src="item.avatar">
                    </v-list-item-avatar>

                    <v-list-item-content>
                      <v-list-item-title v-html="item.memberId"></v-list-item-title>
                      <v-list-item-subtitle v-html="item.calcPrice"></v-list-item-subtitle>
                    </v-list-item-content>

                    <div class="checkBox">
                      <v-checkbox v-model="checkbox" color="light-green" :value="item.dutchId"></v-checkbox>
                    </div>
