    ...
    <div v-if="items.length != 0">
      ...
      >
        <v-list-item three-line>
          ...
          <v-list-item-content>
            <div class="text-overline mt-6">
              {{ item.memberId }}
              <hr />
              {{ item.startDate | moment("YYYY-MM-DD HH:mm") }} ~
              {{ item.endDate | moment("YYYY-MM-DD HH:mm") }} 까지 투표 진행
            </div>
        ...
        <div class="vote">
          <div class="voteContent">
            <v-card v-if="result == false && vote != item.voteId">
              <v-card-actions class="pl-6 pt-5">{{
                item.topic
              }}</v-card-actions>
              <v-row>
            ...
            >
              ...
              <v-progress-linear
                height="25"
                color="success"
                :value="calcProgress(item.cnt)"
              >
                <strong
                  >{{
                    item.cnt == 0 ? 0 : Math.ceil(calcProgress(item.cnt))
                  }}%</strong
                >
              </v-progress-linear>
            </v-card-text>
          ...
          <div v-else-if="itemSelectList[idx]['memberId'] === memberId">
            <v-card-actions class="mr-5">
              ...
              <v-btn
                depressed
                color="success"
                v-if="item.voteId != vote"
                @click="
                  selectCheck(
                    item.voteId,
                    itemSelectList[idx]['itemSelect'],
                    selectItemList[idx]['itemSelect']
                  )
                "
                >투표하기</v-btn
              >
        ...
        <div v-else>
          <v-card-actions class="mr-5">
            <v-spacer></v-spacer>
            <v-btn v-if="item.voteId != vote" color="error" depressed disabled
              >종료가된 투표입니다.</v-btn
            >
            <v-btn
              depressed
  ...
  methods: {
        ...
        .finally((response) => {
          ...
          for (let i = 0; i < vm.items.length; i++) {
            vm.axios
              ...
              .then((response) => {
                this.profile.push({
                  avatar: require(`@/assets/image/user/${response.data}`),
                });
              })
              .catch((err) => {
                console.log(err);
    ...
    selectCheckItem() {
      this.axios
        ...
        .then((resp) => {
          console.log(resp);
          this.selectItemList = resp.data.reverse();
        })
        .catch((err) => {
          console.log(err);
        });
