    ...
    <div class="container">
      ...
      <div v-if="isBookmark == true">
        <div class="gallery">
          <div class="gallery-item" v-for="(bookmark, i) in bookmarks" :key="i">
            <v-img
              @click.stop="goFeedDetail(bookmark.postId, bookmark.memberId)"
              aspect-ratio="1.2"
              :src="
                require(`@/assets/image/sns/${bookmark.postId}/${bookmark.thumbnail}`)
              "
            />
            <v-btn
              v-if="isCollections.length > 1 && catgName !== 'default'"
      ...
      <div class="wrap_box" v-else>
        <v-img
          ...
          width="100"
          src="@/assets/image/sns/default/warn.jpg"
          alt="no_result"
        ></v-img>
        <div class="text_box">
          <p>
            저장된 피드가 없습니다!<br />마음에 드시는 피드 저장부터
            부탁드립니다🙏
          </p>
        </div>
      </div>
        ...
        <v-card>
          ...
          <v-card-actions>
            <v-row class="ma-4 justify-space-around">
              <v-btn text @click="changeCatg()"> 저장 </v-btn>
...
export default {
  ...
  methods: {
    ...
    changeCatg() {
      this.axios
        ...
        .then((res) => {
          ...
          this.$swal(
            "' " +
              this.selectedCollection.catgName +
              " '컬렉션으로 이동이 완료되었습니다.🙏"
          );
          this.dialog2 = !this.dialog2;
    ...
    goFeedDetail(postId, writer) {
      ...
      this.$router.push({
        path: "/snsFeedDetail",
        query: { postId: postId, writer: writer },
      });
    },
  },
};
</script>
<style scoped lang="css" src="@/assets/css/sns/BookmarkDetail.css"></style>
