<template>
  <div id="learn-page-box">
    <v-sheet
      ...
      style="position: relative; background-color: #1d202e"
    >
      ...
      >
        <v-expansion-panels v-model="panel" multiple flat>
          <h1 class="panel-header-title">제목</h1>
          <v-expansion-panel v-for="(item, i) in items" :key="i">
            <v-expansion-panel-header
              class="panel-header"
              @click="changePanelHeader"
            >
            ...
            <v-expansion-panel-content class="panel-content">
              <v-list dense nav>
                ...
                >
                  ...
                  <div v-if="child.viewProgress >= 100">
                    <v-icon
                      :class="
                        child.currId == currId
                          ? 'panel-list-item-time-2'
    ...
    <v-progress-linear :value="knowledge" color="#AAABB7" height="20" readonly>
      <strong style="font-size: 0.9em"
        >전체 진도율 {{ Math.ceil(knowledge) }}%</strong
      >
    </v-progress-linear>
    <v-bottom-navigation color="primary" horizontal class="bottom-nav">
      <v-row>
        ...
        >
          <v-btn
            @click.stop="
              $router
                .push({ path: '/learn/' + (Number(currId) - 1) })
                .catch(() => {
                  $router.go(0);
                })
            "
          >
            <v-icon color="white" style="margin-left: 15px"
              >mdi-chevron-left</v-icon
            >
            <span
              class="bottom-nav-text"
              style="margin-left: 20px; font-size: 1.1em"
              >이전학습</span
            >
          </v-btn>
        </div>
          ...
          >
            <v-tab key="2">
              <v-btn @click="questForm">
                ...
                <v-icon color="white" class="bottom-nav-text-2"
      ...
      >
        ...
        >
          <span
            class="bottom-nav-text"
            style="margin-right: 20px; font-size: 1.1em"
            >다음학습</span
          >
          ...
          <div>
            <v-card class="d-flex justify-end align-center" flat tile>
              ...
              <div class="text-center mb-3" style="padding-right: 20px">
                <v-btn outlined color="#2b2b2b" @click="clickWriteBtn">
                  질문 작성하기
                </v-btn>
              </div>
              <v-card tile flat>
            ...
            <v-card class="mx-auto" outlined>
              <v-card-text>
                <v-row>
                  ...
                  <v-col cols="11">
                    <span
                      ...
                      @click="
                        $router
                          .push({ path: '/snsUserFeed?memId=' + rv.memberId })
                          .catch(() => {
                            $router.go(0);
                          })
                      "
                    ...
                    <span
                      style="font-size: 1em; color: gray; padding-left: 7px"
                      >{{ replaceDate(rv.writeDate) }}</span
                    >
                    <div
                      style="
                        font-size: 1.3em;
                ...
                <div style="padding-top: 20px">
                  <v-row
                    v-if="rv.memberId == $store.state.id"
        ...
        <div v-if="questList.length != 0">
          <div>
            <v-card class="d-flex justify-end align-center" flat tile>
              <!-- 작성하기 버튼 -->
              <div class="text-center mb-3" style="padding-right: 20px">
                <v-btn outlined color="#2b2b2b" @click="sheet = true">
                  노트 작성하기
                </v-btn>
              </div>
              <v-card tile flat>
...
export default {
  ...
  methods: {
    pushCheck(item) {
      this.$router.push({ path: "/learn/" + item.currId }).catch(() => {
        $router.go(0);
      });
    },
    getCurrInfo() {
    ...
    deleteContent(idx) {
      ...
      this.axios
        ...
        .then((res) => {
          if (res.status == 200) {
            this.$swal("삭제 완료!", "작성한 QnA를 삭제하였습니다.", "success");
            this.sheet = false;
            if (this.form.type == 2) {
  ...
  mounted() {
    document
      .querySelector(".art-video")
      .addEventListener("timeupdate", this.timeUpdate);
    document
      .querySelector(".art-video")
      .addEventListener("ended", this.onEnded(this.currId));
  },
};
</script>
<style>
